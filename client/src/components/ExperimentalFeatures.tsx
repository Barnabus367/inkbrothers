...
export function TattooVisionAI() {
  ...
  const startVision = async () => {
    ...
    try {
      ...
      setError("Kamera-Zugriff verweigert. Bitte erlauben Sie den Kamera-Zugriff für KI-Analyse.");
  ...
  const startAnalysis = () => {
    ...
    analysisSteps.forEach(({ step, text, delay }) => {
      setTimeout(() => {
        setAnalysisResults(prev => [...prev, { step, text, timestamp: Date.now() }]);
        if (step === 4) {
          setIsAnalyzing(false);
      ...
      {!isActive ? (
        <div className="space-y-4">
          ...
          <div className="text-center text-gray-400 text-sm">
            <p>Die KI analysiert Ihre Körperstelle und empfiehlt optimale Tattoo-Stile</p>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {analysisResults.length > 0 && (
            <div className="bg-gray-800/50 rounded-lg p-4">
              <h4 className="text-white font-medium mb-2">Analyse-Fortschritt:</h4>
...
export function PainSimulator() {
  ...
  const bodyParts = [
    { id: "arm", name: "Oberarm", pain: 3, nerves: "wenige", description: "Muskulöse Region, gut gepolstert" },
    { id: "forearm", name: "Unterarm", pain: 4, nerves: "mäßig", description: "Mehr Knochen spürbar, moderate Schmerzen" },
    { id: "ribs", name: "Rippen", pain: 8, nerves: "viele", description: "Direkt auf Knochen, sehr schmerzhaft" },
    { id: "back", name: "Rücken", pain: 4, nerves: "wenige", description: "Große Fläche, variiert je nach Bereich" },
    { id: "ankle", name: "Knöchel", pain: 7, nerves: "viele", description: "Dünne Haut, viele Nervenenden" },
    { id: "shoulder", name: "Schulter", pain: 5, nerves: "mäßig", description: "Muskulös aber bewegungsaktiv" },
    { id: "wrist", name: "Handgelenk", pain: 8, nerves: "sehr viele", description: "Sehr empfindlich, dünne Haut" },
    { id: "thigh", name: "Oberschenkel", pain: 3, nerves: "wenige", description: "Großer Muskel, wenig schmerzhaft" },
    { id: "shin", name: "Schienbein", pain: 9, nerves: "sehr viele", description: "Direkt auf Knochen, extrem schmerzhaft" },
    { id: "neck", name: "Nacken", pain: 7, nerves: "viele", description: "Empfindliche Region, viele Nerven" }
  ];
  ...
  const calculatePainPrediction = (selectedPart: any) => {
    ...
    const sizeModifier = sizeMultipliers[tattooSize as keyof typeof sizeMultipliers].factor;
    const toleranceModifier = (10 - personalTolerance) / 10;
    const durationModifier = sessionDuration > 3 ? 1.2 : 1.0;
    
    const calculatedPain = Math.min(10, Math.round(basePain * sizeModifier * toleranceModifier * durationModifier));
  ...
  return (
    <div className="bg-black/90 backdrop-blur-sm border border-red-500/30 rounded-lg p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="px-2 py-1 bg-orange-500/20 text-orange-400 text-xs rounded-full">ADVANCED</span>
      </div>
      
      <div className="space-y-4">
        {/* Body Part Selection */}
        <div>
          <label className="block text-white font-medium mb-2">Körperstelle wählen</label>
          <select
            value={bodyPart}
        ...
        {/* Tattoo Size */}
        <div>
          <label className="block text-white font-medium mb-2">Tattoo-Größe</label>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="w-full bg-gray-700 rounded-full h-4 relative overflow-hidden">
            <div 
              className={`h-4 rounded-full transition-all duration-500 relative ${
                painLevel <= 3 ? 'bg-gradient-to-r from-green-500 to-green-400' :
                painLevel <= 6 ? 'bg-gradient-to-r from-yellow-500 to-orange-400' : 
        ...
        {finalPrediction && (
          <div className="bg-gray-800/50 rounded-lg p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <p className="text-gray-300"><strong>Region:</strong> {finalPrediction.bodyPart}</p>
                <p className="text-gray-300"><strong>Geschätzte Pausen:</strong> {finalPrediction.estimatedBreaks}</p>
                <p className="text-gray-300"><strong>Nachsorge:</strong> {finalPrediction.aftercare}</p>
              </div>
              
              <div className="space-y-1">
                <p className="text-white font-medium">Empfehlungen:</p>
                {finalPrediction.recommendations.map((rec: string, i: number) => (
...
export function NeuralTattooDesigner() {
  ...
  const styles = [
    { id: "neo-traditional", name: "Neo-Traditional", description: "Moderne Interpretation klassischer Tattoos" },
    { id: "realism", name: "Realism", description: "Fotorealistische Darstellungen" },
    { id: "geometric", name: "Geometric", description: "Mathematische Formen und Muster" },
    { id: "blackwork", name: "Blackwork", description: "Reine schwarze Tinte, hoher Kontrast" },
    { id: "watercolor", name: "Watercolor", description: "Aquarell-ähnliche Farbverläufe" },
    { id: "minimalist", name: "Minimalist", description: "Reduziert auf das Wesentliche" },
    { id: "biomechanical", name: "Biomechanical", description: "Organische und mechanische Elemente" },
    { id: "tribal", name: "Tribal", description: "Traditionelle Stammesmuster" }
  ];

  const sizes = [
    { id: "small", name: "Klein", description: "2-5cm", price: "150-300 CHF" },
    { id: "medium", name: "Mittel", description: "5-15cm", price: "300-800 CHF" },
    { id: "large", name: "Groß", description: "15-30cm", price: "800-1500 CHF" },
    { id: "sleeve", name: "Sleeve", description: "Ganzer Arm", price: "1500-3000 CHF" }
  ];

  const complexities = [
    { id: "simple", name: "Einfach", description: "Klare Linien, wenig Details" },
    { id: "medium", name: "Mittel", description: "Moderate Details, Schattierungen" },
    { id: "complex", name: "Komplex", description: "Hochdetailliert, mehrere Ebenen" },
    { id: "masterpiece", name: "Meisterwerk", description: "Höchste Kunstfertigkeit" }
  ];
  ...
  const generateDesign = () => {
    ...
    steps.forEach(({ step, text, delay }) => {
      setTimeout(() => {
        setGenerationSteps(prev => [...prev, { step, text, timestamp: Date.now() }]);
        if (step === 7) {
          setTimeout(() => {
            generateFinalDesign();
  ...
  const generateFinalDesign = () => {
    ...
    const selectedComplexityData = complexities.find(c => c.id === selectedComplexity);
  ...
  const generateDesignElements = () => {
    ...
    if (keywords.some(k => ['tier', 'wolf', 'löwe', 'adler', 'schlange'].includes(k))) {
    ...
    if (keywords.some(k => ['geom', 'dreieck', 'kreis', 'mandala'].includes(k))) {
    ...
    return elements.length > 0 ? elements : ["Individuelles Design-Konzept", "Stilspezifische Ausführung"];
  };

  const generateTechnicalSpecs = () => {
    return {
      needleConfiguration: selectedComplexity === 'complex' ? 'Liner + Shader' : 'Standard Liner',
      inkColors: selectedStyle === 'blackwork' ? 'Schwarz' : 'Schwarz + Farben',
      sessionBreaks: calculateSessions() > 1 ? 'Ja, alle 2-3 Stunden' : 'Nach Bedarf',
      healingTime: selectedSize === 'sleeve' ? '4-6 Wochen' : '2-4 Wochen'
    };
  };

  const calculateEstimatedTime = () => {
    const baseTime = sizes.find(s => s.id === selectedSize)?.id === 'small' ? 2 : 
                     sizes.find(s => s.id === selectedSize)?.id === 'medium' ? 4 :
                     sizes.find(s => s.id === selectedSize)?.id === 'large' ? 8 : 12;
    
    const complexityMultiplier = selectedComplexity === 'simple' ? 0.8 :
                                selectedComplexity === 'medium' ? 1.0 :
                                selectedComplexity === 'complex' ? 1.5 : 2.0;
  ...
  const bookConsultation = (design: any) => {
    ...
    alert(`Beratungstermin für "${design.prompt}" angefragt. David wird sich melden!`);
  };
  ...
  return (
    <div className="bg-black/90 backdrop-blur-sm border border-red-500/30 rounded-lg p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 text-xs rounded-full">KI-POWERED</span>
      </div>
      
      <div className="space-y-6">
        {/* Prompt Input */}
        <div>
          <label className="block text-white font-medium mb-2">Beschreiben Sie Ihr Wunschtattoo</label>
          <textarea
            value={prompt}
        ...
        {/* Style Selection */}
        <div>
          <label className="block text-white font-medium mb-2">Tattoo-Stil</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-white font-medium mb-2">Komplexität</label>
            <select
        ...
        {generationSteps.length > 0 && (
          <div className="bg-gray-800/50 rounded-lg p-4">
            <h4 className="text-white font-medium mb-3">Generation Progress:</h4>
            <div className="space-y-2">
        ...
        {finalDesign && (
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg p-6 border border-cyan-500/30">
            <div className="flex justify-between items-start mb-4">
              <h4 className="text-white font-bold text-lg">Generiertes Design</h4>
              <span className="text-xs text-gray-400">{finalDesign.createdAt}</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">{finalDesign.style.description}</p>
                </div>
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-cyan-400 font-medium">Technische Details:</p>
                  <div className="text-white text-sm space-y-1">
                    <p><strong>Größe:</strong> {finalDesign.size.name} ({finalDesign.size.description})</p>
                    <p><strong>Geschätzte Zeit:</strong> {finalDesign.estimatedTime}h</p>
                    <p><strong>Sitzungen:</strong> {finalDesign.estimatedSessions}</p>
                    <p><strong>Preis:</strong> {finalDesign.size.price}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-white text-sm space-y-1">
                    <p><strong>Nadel:</strong> {finalDesign.technical.needleConfiguration}</p>
                    <p><strong>Farben:</strong> {finalDesign.technical.inkColors}</p>
                    <p><strong>Heilung:</strong> {finalDesign.technical.healingTime}</p>
                  </div>
                </div>
          ...
          <div className="bg-gray-800/30 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {designHistory.slice(0, 3).map((design, i) => (
                <div key={design.id} className="flex justify-between items-center text-sm">
                  <span className="text-gray-300 truncate">{design.prompt}</span>
